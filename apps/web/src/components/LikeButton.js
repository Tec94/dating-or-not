...
export default function LikeButton({ betId, initialLikes = 0, initialLiked = false, onLiked }) {
    ...
    return (_jsx(motion.button, { onClick: like, className: `px-3 py-1 rounded-full text-sm transition-colors ${justLiked ? 'bg-brand-green text-black' : 'bg-white/10 hover:bg-white/15'}`, disabled: isLoading || optimisticLocked, whileTap: { scale: 0.95 }, animate: justLiked ? { scale: [1, 1.15, 1] } : {}, transition: { duration: 0.3 }, children: _jsxs("span", { children: [liked ? '💚' : '❤️', " ", likes] }) }));
}
